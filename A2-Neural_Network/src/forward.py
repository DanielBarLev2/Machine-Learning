...
def l_model_forward(x_train: np.array, parameters: dict) -> tuple[np.array, list]:
    """
    Description: Implement forward propagation for the [LINEAR->RELU]*(L-1)->LINEAR->SOFTMAX computation

    Input:
    data_x – the data, numpy array of shape (input size, number of examples)
    ...
    activation_last, cache = linear_activation_forward(activation_prev=activation, w=parameters[f'W{last_layer}'],
                                                      b=parameters[f'b{last_layer}'], activation_fn="softmax")
